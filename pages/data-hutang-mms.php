...
$totalProyek = newQuery("get_var","SELECT COUNT(*) FROM tb_po WHERE $cond2 $cond AND IsLD='0'");
$grandTotalProyek = newQuery("get_var","SELECT SUM(GrandTotal) FROM tb_po WHERE $cond2 $cond AND IsLD='0'");
$totalTerbayar = newQuery("get_var","SELECT SUM(TotalPembayaran) FROM tb_po WHERE $cond2 $cond AND IsLD='0'");
$totalPiutang = newQuery("get_var","SELECT SUM(Sisa) FROM tb_po WHERE $cond2 $cond AND IsLD='0'");
?>
<?php include "pages/header.php"; ?>
    <section class="section section-body bg-grey">
        <section id="overview" class="grid-hero container">
            ...
            <h5>Hutang PO MMS<small>Pengelolaan data hutang PO MMS</small></h5>
            <?php if(isset($notif)){ ?>
            ...
            <table class="table new-table">
                ...
                <body>
                    ...
                    if($no_po!=""){
                        $query = newQuery("get_results","SELECT * FROM tb_po WHERE NoPo='$no_po' AND IsLD='0' ORDER BY NoPo ASC");
                    } else {
                        $query = newQuery("get_results","SELECT * FROM tb_po WHERE IsLD='0' AND $cond2 $cond ORDER BY NoPo ASC");
                    }
                    ...
                    if($query){
                        ...
                        foreach($query as $data){
                            ...
                            ?>
                            <tr>
                                <td><a href="../../MMS/#/purchase-order/detail/<?php echo $data->NoPo; ?>" target="_blank"><?php echo $data->NoPo; ?></a></td>
