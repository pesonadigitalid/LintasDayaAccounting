...
$totalProyek = newQuery("get_var","SELECT COUNT(*) FROM tb_proyek_invoice WHERE IDInvoice>0 $cond");
if(!$totalProyek) $totalProyek=0;

$grandTotalProyek = newQuery("get_var","SELECT GrandTotal FROM tb_proyek WHERE IDProyek>0 $cond");
if(!$grandTotalProyek) $grandTotalProyek=0;
$totalPiutang = newQuery("get_var","SELECT SisaPembayaran FROM tb_proyek WHERE IDProyek>0 $cond");
if(!$totalPiutang) $totalPiutang=0;

$totalTerbayar=0;
$query = newQuery("get_results","SELECT * FROM tb_proyek_invoice WHERE IDInvoice>0 $cond");
if($query){
...
<?php include "pages/header.php"; ?>
    <section class="section section-body bg-grey">
        <section id="overview" class="grid-hero container">
            ...
            <h5>Detail Pembayaran Proyek<small>Daftar invoice proyek</small></h5>
            ...
            <table class="table new-table">
                ...
                <body>
                    <?php
                    $query = newQuery("get_results","SELECT *, DATE_FORMAT(Tanggal, '%d/%m/%Y') AS TanggalID FROM tb_proyek_invoice WHERE IDInvoice>0 $cond ORDER BY Tanggal ASC, NoInv ASC");
                    if($query){
                        $total = 0;
                        $ppn = 0;
                        $grandtotal = 0;
