...
$periode = "Departement: ".$dep->NamaDepartement.". Tahun: ".$tahun;
...
<body class="center">
    ...
    <table class="tbLabaRugi" style="width: auto">
        ...
        <tr>
            ...
            $query = newQuery("get_results","SELECT * FROM tb_proyek WHERE Tahun='$tahun' AND IDDepartement='$departement' ORDER BY KodeProyek ASC");
            if($query){
        ...
        <tr>
            ...
            foreach($ProyekArray as $id){
                $query = $db->get_results("SELECT a.*, DATE_FORMAT(a.Tanggal,'%d/%m/%Y') AS TanggalID FROM tb_po a, tb_supplier b WHERE a.IDProyek='$id' AND a.JenisPO='1' AND a.IDSupplier=b.IDSupplier");
                $material1 = 0;
                if($query){
                    foreach($query as $data){
        ...
        <tr>
            ...
            foreach($ProyekArray as $id){
                ...
                $query = $db->get_results("SELECT a.*, DATE_FORMAT(a.Tanggal,'%d/%m/%Y') AS TanggalID FROM tb_po a, tb_supplier b WHERE a.IDProyek='$id' AND a.JenisPO='2' AND a.IDSupplier=b.IDSupplier");
                if($query){
                    foreach($query as $data){
        ...
        <tr>
            ...
            foreach($ProyekArray as $id){
                ...
                $query = $db->get_results("SELECT a.*, DATE_FORMAT(a.Tanggal,'%d/%m/%Y') AS TanggalID FROM tb_po a, tb_supplier b WHERE a.IDProyek='$id' AND a.JenisPO='3' AND a.IDSupplier=b.IDSupplier");
                if($query){
                    foreach($query as $data){
                        if(($data->Sisa>0 && $data->Sisa<1) || $data->Sisa<0) $sisa = 0; else  $sisa = $data->Sisa;
                ...
                ?><td style="text-align: center;text-align:right;border-bottom: solid 1px #333;padding-bottom: 5px;"><?php echo number_format($overhead1,2); ?></td><?php
        ...
        <tr>
            ...
            foreach($ProyekArray as $key=>$id){
                $totalPengeluaran = $BiayaMaterialArray[$key]+$PengirimanArray[$key]+$TenagaArray[$key]+$OverheadArray[$key];
