...
$pelanggan = $this->validasi->validInput($_GET['pelanggan']);
$tipe = $this->validasi->validInput($_GET['tipe']);


$pel = newQuery("get_row","SELECT * FROM tb_pelanggan WHERE IDPelanggan='$pelanggan'");
...
$periode = "Pelanggan: ".$pel->NamaPelanggan.". Tahun: ".$tahun;
...
<body class="center">
    <?php if($tipe=="1"){ ?>
    <h1 class="blue">Proyeksi Project Per Pelanggan</h1>
    ...
    <table class="tbLabaRugi" style="width: auto">
        ...
        <tr>
            ...
            $query = newQuery("get_results","SELECT * FROM tb_proyek WHERE Tahun='$tahun' AND IDClient='$pelanggan' ORDER BY KodeProyek ASC");
            if($query){
                foreach($query as $data){
                    ?><td width="90" style="text-align: center;font-weight: bold;border-bottom: solid 1px #333;padding-bottom: 5px;" class="red"><?php echo $data->KodeProyek; ?></td><?php
                    ...
                    $q = $db->get_results("SELECT *, DATE_FORMAT(Tanggal,'%d/%m/%Y') AS TanggalID FROM tb_proyek_invoice WHERE IDProyek='$id' ORDER BY IDProyek ASC");
                    if($q){
        ...
        <tr>
            ...
            foreach($ProyekArray as $id){
                $query = $db->get_results("SELECT a.*, DATE_FORMAT(a.Tanggal,'%d/%m/%Y') AS TanggalID FROM tb_po a, tb_supplier b WHERE a.IDProyek='$id' AND a.JenisPO='1' AND a.IsAccidental='0' AND a.IDSupplier=b.IDSupplier");
                $material1 = 0;
                if($query){
                    foreach($query as $data){
                ...
                $query = $db->get_results("SELECT a.*, b.*, DATE_FORMAT(a.Tanggal,'%d/%m/%Y') AS TanggalID FROM tb_jurnal a, tb_jurnal_detail b WHERE a.IDJurnal=b.IDJurnal AND b.IDRekening='45' AND b.Debet>0 AND a.IDProyek='$id' AND a.NoRef=''");
                if($query){
                    foreach($query as $data){
                        if($data->NoRef==''){
        ...
        <tr>
            ...
            foreach($ProyekArray as $id){
                ...
                $query = $db->get_results("SELECT a.*, DATE_FORMAT(b.Tanggal,'%d/%m/%Y') AS TanggalID, b.IDPengiriman FROM tb_pengiriman_detail a, tb_pengiriman b WHERE a.NoPengiriman=b.NoPengiriman AND b.IDProyek='$id' AND StokFrom='0' GROUP BY a.NoPengiriman");
                if($query){
                    foreach($query as $data){
        ...
        <tr>
            ...
            foreach($ProyekArray as $id){
                ...
                $query = $db->get_results("SELECT a.*, DATE_FORMAT(a.Tanggal,'%d/%m/%Y') AS TanggalID FROM tb_po a, tb_supplier b WHERE a.IDProyek='$id' AND a.JenisPO='2' AND a.IsAccidental='0' AND a.IDSupplier=b.IDSupplier");
                if($query){
                    foreach($query as $data){
        ...
        <tr>
            ...
            foreach($ProyekArray as $id){
                ...
                $query = $db->get_results("SELECT a.*, DATE_FORMAT(a.Tanggal,'%d/%m/%Y') AS TanggalID FROM tb_po a, tb_supplier b WHERE a.IDProyek='$id' AND a.JenisPO='3' AND a.IsAccidental='0' AND a.IDSupplier=b.IDSupplier");
                if($query){
                    foreach($query as $data){
                        if(($data->Sisa>0 && $data->Sisa<1) || $data->Sisa<0) $sisa = 0; else  $sisa = $data->Sisa;
