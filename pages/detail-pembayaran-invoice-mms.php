...
if($invoice!=""){
    $cond = " AND NoRef='$invoice' AND Tipe='4'";
}

$dataInvoice = newQuery("get_row","SELECT * FROM tb_penjualan_invoice WHERE IDInvoice='$invoice'");

$totalProyek = newQuery("get_var","SELECT COUNT(*) FROM tb_jurnal WHERE NoRef='$invoice' AND Tipe='4'");
if(!$totalProyek) $totalProyek=0;
$grandTotalProyek = newQuery("get_var","SELECT GrandTotal FROM tb_penjualan_invoice WHERE IDInvoice='$invoice'");
if(!$grandTotalProyek) $grandTotalProyek=0;

$totalPiutang = newQuery("get_var","SELECT SUM(Sisa) FROM tb_penjualan_invoice WHERE IDInvoice='$invoice'");
if(!$totalPiutang) $totalPiutang=0;

$totalTerbayar=0;
$query = newQuery("get_results","SELECT * FROM tb_penjualan_invoice WHERE IDInvoice='$invoice'");
if($query){
...
<?php include "pages/header.php"; ?>
    <section class="section section-body bg-grey">
        <section id="overview" class="grid-hero container">
            ...
            <h5>Detail Pembayaran No Invoice <?php echo $dataInvoice->NoInvoice; ?></h5>
