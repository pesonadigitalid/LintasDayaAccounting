...
$dName = newQuery("get_var", "SELECT NamaDepartement FROM tb_departement WHERE IDDepartement='$departement'");
if (!$dName) $dName = "Umum";
...
<body class="center">
    <?php if ($print_header == '1') { ?>
        <h1 class="blue">Laba Rugi Departement <?php echo $headerDisplay; ?></h1>
    ...
    <?php if ($print_detail == '1') { ?>
        ...
        <table class="tbLabaRugi" style="max-width: 500px">
            <tr>
                <td width="400" style="font-weight: bold;" class="red">Detail Report</td>
                <td width="100" style="text-align: right;font-weight: bold;border-bottom: solid 1px #333;padding-bottom: 5px;" class="red">Saldo</td>
            </tr>
            <tr>
                <td class="labelHeader">Pendapatan</td>
            ...
            if ($query) {
                foreach ($query as $data) {
                    if ($data->Tipe == "D") {
                        if (CheckSaldo($data->IDRekening, $condDate, $db, "KREDIT") > 0) {
            ?>
                            <tr>
                                <td class="labelHeader2 deep1"><?php echo $data->KodeRekening . "&nbsp;&nbsp;&nbsp;" . ucwords(strtolower($data->NamaRekening)); ?></td>
                                <td style="text-align: right;"></td>
                            ...
                            <?php
                            $queryDetail = newQuery("get_results", "SELECT a.*, DATE_FORMAT(a.Tanggal,'%d/%m/%Y') AS TanggalID, b.NoBukti FROM tb_jurnal_detail a, tb_jurnal b WHERE a.`IDJurnal`=b.`IDJurnal` AND b.`IDDepartement`='$departement' AND a.`IDRekening`='" . $data->IDRekening . "' $condDate ORDER BY Tanggal ASC, JurnalRef ASC");
                            if ($queryDetail) {
                                foreach ($queryDetail as $dataDetail) {
                                    $jurnal = newQuery("get_row", "SELECT * FROM tb_jurnal WHERE IDJurnal='" . $dataDetail->IDJurnal . "'");
                                    if ($jurnal->NoRef != '' && $jurnal->Tipe == $idTipe) {
                                    ...
                                    $jurnalTandingan = newQuery("get_row", "SELECT b.* FROM tb_jurnal_detail a, tb_master_rekening b WHERE a.IDJurnal='" . $dataDetail->IDJurnal . "' AND a.IDJurnalDetail!='" . $dataDetail->IDJurnalDetail . "' AND a.IDRekening=b.IDRekening");
                                    $pendapatan += $dataDetail->Kredit;
                                    $proyek = newQuery("get_row", "SELECT * FROM tb_proyek WHERE IDProyek='" . $jurnal->IDProyek . "'");
                            ?>
                                    <tr>
                                        <td class="deep2">
                                            <table>
                                                <tr>
                                                    <td width="80"><?php echo $dataDetail->TanggalID . "<br/>" . $dataDetail->NoBukti; ?></td>
                                                    <td width="80"><?php echo $proyek->KodeProyek . " / " . $proyek->Tahun; ?></td>
                            ...
                            ?>
                            <tr>
                                <td class="labelHeader2 deep1"></td>
                                <td style="text-align: right;font-weight: bold;border-top: solid 1px #333;padding-bottom: 5px;" class="blue"><?php echo CheckSaldo($data->IDRekening, $condDate, $db, "KREDIT"); ?></td>
                            </tr>
                            <?php
                        }
                    } else {
                        ...
                        if ($querySub) {
                            foreach ($querySub as $dataSub) {
                                if ($dataSub->Tipe == "D") {
                                    if (CheckSaldo($dataSub->IDRekening, $condDate, $db, "KREDIT") > 0) {
                            ?>
                                        <tr>
                                            <td class="labelHeader2 deep1"><?php echo $dataSub->KodeRekening . "&nbsp;&nbsp;&nbsp;" . ucwords(strtolower($dataSub->NamaRekening)); ?></td>
                                            <td style="text-align: right;"></td>
                                        ...
                                        <?php
                                        $queryDetail = newQuery("get_results", "SELECT a.*, DATE_FORMAT(a.Tanggal,'%d/%m/%Y') AS TanggalID, b.NoBukti FROM tb_jurnal_detail a, tb_jurnal b WHERE a.`IDJurnal`=b.`IDJurnal` AND b.`IDDepartement`='$departement' AND a.`IDRekening`='" . $dataSub->IDRekening . "' $condDate ORDER BY Tanggal ASC, JurnalRef ASC");
                                        if ($queryDetail) {
                                            foreach ($queryDetail as $dataDetail) {
                                                $jurnal = newQuery("get_row", "SELECT * FROM tb_jurnal WHERE IDJurnal='" . $dataDetail->IDJurnal . "'");
                                                if ($jurnal->NoRef != '' && $jurnal->Tipe == $idTipe) {
                                        ...
                                        <tr>
                                            <td class="labelHeader2 deep1"></td>
                                            <td style="text-align: right;font-weight: bold;border-top: solid 1px #333;padding-bottom: 5px;" class="blue"><?php echo CheckSaldo($dataSub->IDRekening, $condDate, $db, "KREDIT"); ?></td>
                                        </tr>
                <?php
                                    }
                                }
            ...
            if ($query) {
                foreach ($query as $data) {
                    if ($data->Tipe == "D") {
                        if (CheckSaldo($data->IDRekening, $condDate, $db, "DEBET") > 0) {
            ?>
                            <tr>
                                <td class="labelHeader2 deep1"><?php echo $data->KodeRekening . "&nbsp;&nbsp;&nbsp;" . ucwords(strtolower($data->NamaRekening)); ?></td>
                                <td style="text-align: right;"></td>
                            ...
                            <tr>
                                <td class="labelHeader2 deep1"></td>
                                <td style="text-align: right;font-weight: bold;border-top: solid 1px #333;padding-bottom: 5px;" class="blue"><?php echo CheckSaldo($data->IDRekening, $condDate, $db, "DEBET"); ?></td>
                            </tr>
                            <?php
                        }
                    } else {
                        ...
                        if ($querySub) {
                            foreach ($querySub as $dataSub) {
                                if ($dataSub->Tipe == "D") {
                                    if (CheckSaldo($dataSub->IDRekening, $condDate, $db, "DEBET") > 0) {
                            ?>
                                        <tr>
                                            <td class="labelHeader2 deep1"><?php echo $dataSub->KodeRekening . "&nbsp;&nbsp;&nbsp;" . ucwords(strtolower($dataSub->NamaRekening)); ?></td>
                                            <td style="text-align: right;"></td>
                                        ...
                                        <tr>
                                            <td class="labelHeader2 deep1"></td>
                                            <td style="text-align: right;font-weight: bold;border-top: solid 1px #333;padding-bottom: 5px;" class="blue"><?php echo CheckSaldo($dataSub->IDRekening, $condDate, $db, "DEBET"); ?></td>
                                        </tr>
                <?php
                                    }
                                }
